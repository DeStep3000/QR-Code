...
def read_square_matrix():
    size = int(input("Введите размерность квадратной матрицы: "))  # Запрашиваем размерность квадратной матрицы у пользователя
    matrix = []
    print("Введите элементы матрицы:")
    for _ in range(size):
        row = []
        for _ in range(size):
            element = int(input())  # Запрашиваем элементы матрицы от пользователя
            row.append(element)
        matrix.append(row)
    return matrix, size

# Функция для выполнения QR-алгоритма над матрицей A
...
# Функция для нахождения собственных значений матрицы A
